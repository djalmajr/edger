...
## Context
- **Problem:** Foundation phase fecha sem enforcement rigoroso de limites body/header, sem testes sistemáticos de paridade Buntime, e sem baselines de performance documentadas (decisão usuário: measurement em PR 12).
- **Objective:** Aplicar hardening de segurança (limites, sanitização, erros de domínio), implementar matriz de compatibilidade Buntime automatizada, portar harness de performance e definir targets/baselines.
- **Value:** Confiança para migração Buntime → edger; critérios objetivos de “foundation complete”; CI bloqueia regressões de contrato.
- **Constraints:** Alinhar HeaderLimits Buntime (100 headers, 64KiB total, 8KiB/value); harness não bloqueia CI lento — tier `#[ignore]` para perf; tree sempre verde sob gate strict.
...
| `docs/compat-matrix.md` | create | Tabela comportamento Buntime ↔ edger + tiers |
...
- `docs/compat-matrix.md` marca cada item: ✅ tested | ⚠️ partial | ❌ gap com rationale.
- `docs/performance-baselines.md` registra números iniciais e targets aspiracionais (<50ms spawn cached per design).
...
- [ ] Suite `tests/compat/` verde cobrindo ≥90% das linhas “must preserve” do design Migration notes.
- [ ] `docs/compat-matrix.md` publicado com status por comportamento.
- [ ] Harness executa e grava baselines em `docs/performance-baselines.md` (mesmo que targets não atingidos — documentar gap).
...
- [ ] Gerar `docs/compat-matrix.md` a partir de checklist rastreável.
...
- `bun test`
- Revisão manual `docs/compat-matrix.md` vs design Migration notes — sem linhas críticas em ❌ sem justificativa