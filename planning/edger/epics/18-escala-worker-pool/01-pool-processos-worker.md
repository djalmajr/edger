...
**completed** (2026-07-03) — pool de N processos por worker mergeado: configuração `concurrency`/`minProcesses`/`maxProcesses`, grupo de processos por worker, roteamento para instância livre com criação sob demanda até o teto, reciclagem por instância e testes dirigidos sem socket. Verificação registrada: `cargo check --workspace --all-targets`, `cargo fmt --all --check`, `cargo clippy -p edger-worker -p edger-orchestrator`, `cargo test -p edger-worker story18`, `cargo test -p edger-orchestrator --test pipeline_integration`.
