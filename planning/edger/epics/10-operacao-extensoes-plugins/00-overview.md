...
| 10.02 Reconcile/reload controlado | `02-reconcile-reload-controlado.md` | Separar dry-run, reconcile e reload seguro sem loader dinamico de Rust | large | completed | 10.01, Epic 08.26 |
...
| 10.04 Validacao local de extensoes | `04-validacao-local-extensoes.md` | Rodar validacao local de modulo/extensao e registrar evidencia | medium | completed | 10.02, 10.03 |

## Epic acceptance criteria

- [x] Inventario operacional lista modulos, origem, capabilities, dependencias, status, diagnostics e configuracao segura.
- [x] Reconcile/reload diferencia o que pode mudar em runtime do que exige restart.
- [x] Manifesto operacional persiste estado suficiente para auditoria local sem loader dinamico.
- [x] Validacao local prova pelo menos uma extensao existente com gates e evidencia versionada.
- [x] Nenhuma implementacao coloca I/O em `edger-core`.
- [x] Gate de planejamento fica verde: `SCRATCH=planning/edger/status/evidence planning/edger/scripts/run-gates.sh`.

## Status

completed (2026-07-01) - Epic 10 fechado. 10.01: inventario operacional root-only expoe `id`, `version`, `kind`, `capabilities`, `dependencies`, `status`, `configSource` e diagnostics redigidos (chaves/valores/paths sensiveis) via `/api/admin/extensions`, com testes cobrindo middleware (gateway) e provider (keyval) e negacao non-root. 10.02: reconcile root-only em `/api/admin/extensions/reconcile` compara o status store persistido com o overlay em memoria, diferencia dry-run/apply, classifica `runtime` versus `restartRequired`, preserva request ID e nao faz dynamic loading. 10.03: manifesto operacional tipado por extensao (`manifest` com `menus`, `hooks`, `provides`, `requirements` e `config` com `keys` por nome + `redacted`/`source`), derivado das capabilities/dependencias declaradas, backward-compatible com registro estatico, documentado no adoc e coberto em `registry_providers.rs`. 10.04: `planning/edger/scripts/extension-validation.py` e `run-gates.sh` validam localmente inventario, manifesto, status, diagnostics e redaction para a extensao `gateway`, registrando evidencia em `planning/edger/status/evidence/extension-validation.txt`.
