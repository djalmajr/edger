...
- Method-map do harness responde 405 com `Allow` ordenado e preserva handlers diretos.
- MCP cobre install (path/base64), list, enable, disable, delete, promote, invoke e eventos filtráveis por worker.
...
- O lowcode-studio deve criar draft com `visibility: internal`, usar force apenas nessa versão (SEMPRE com `x-edger-expected-revision` da última resposta — install, list ou files), publicar release como nova versão pública e então promover.
