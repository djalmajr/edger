...
#[tokio::test]
async fn disabled_candidate_runs_release_before_health_and_activation() {
    let root = tempfile::tempdir().unwrap();
    let app = build_pipeline(state_with_root(root.path().to_path_buf()));
    let package = zip_package(&[
        (
            "manifest.yaml",
            "name: release-health-app\nversion: \"1.0.0\"\nentrypoint: index.ts\nkind: fetch\nrelease: \"true\"\nhealthCheck:\n  path: /health\n  mode: on-deploy\n  timeout: 2s\n",
        ),
        (
            "index.ts",
            "Deno.serve(() => new Response('healthy'));",
        ),
    ]);
    ...
    assert_eq!(status, StatusCode::CREATED, "unexpected body: {text}");
    assert_eq!(json["release"], "completed");
    assert_eq!(json["health"], "passed");
    assert_eq!(json["activation"], "active");
    assert!(
        std::path::Path::new(json["source"].as_str().unwrap())
            .join(".edger-release")
            .is_file(),
        "release marker must be created while the candidate is still gated"
    );
}

#[tokio::test]
async fn core_names_install_into_overlay_and_regular_workers_into_user_root() {
    let bundled = tempfile::tempdir().unwrap();
    let overlay = tempfile::tempdir().unwrap();
    let user = tempfile::tempdir().unwrap();
    let bundled_cpanel = bundled.path().join("cpanel");
    fs::create_dir(&bundled_cpanel).unwrap();
    fs::write(
        bundled_cpanel.join("manifest.yaml"),
        "name: cpanel\nversion: \"1.0.0\"\nentrypoint: index.html\nkind: static\n",
    )
    .unwrap();
    fs::write(bundled_cpanel.join("index.html"), "bundled").unwrap();
    let app = build_pipeline(state_with_roots(
        bundled.path().to_path_buf(),
        overlay.path().to_path_buf(),
        user.path().to_path_buf(),
    ));
    let core_package = zip_package(&[
        (
            "manifest.yaml",
            "name: cpanel\nversion: \"2.0.0\"\nentrypoint: index.html\nkind: static\n",
        ),
    ...
    .await;
    assert_eq!(status, StatusCode::CREATED, "unexpected body: {text}");
    assert_eq!(json["origin"], "core_overlay");
    assert_eq!(json["defaultVersion"], "2.0.0");
    assert!(std::path::Path::new(json["source"].as_str().unwrap())
        .canonicalize()
        .unwrap()
        .starts_with(overlay.path().canonicalize().unwrap()));
    assert!(!user.path().join("cpanel").exists());
