    ...
    <div>
      ...
      <div class="grid gap-3">
        ${paginatedGroups.map((group) => {
          ...
          const isCore = group.versions.some((worker) => worker.origin && worker.origin !== "user");
          const coreOrigin = group.versions.some((worker) => worker.origin === "core_overlay") ? "Core overlay" : "Core bundled";
          ...
          return html`
            <${Collapsible} className="border-border bg-card rounded-xl border" key=${group.name} open=${isOpen}>
              <${CollapsibleTrigger} className="flex w-full items-center gap-2 px-3 py-3 text-left sm:gap-3 sm:px-4" interactive=${false} onClick=${(event) => event.preventDefault()}>
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    ${isCore && html`<${Badge} variant="outline">${coreOrigin}<//>`}
                  </div>
              ...
              ${group.versions.map((worker) => {
                ...
                const canDisable = !isCore || enabledVersions > 1;
                return html`
